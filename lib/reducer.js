...
var makeNormalizedPayloadSource = function makeNormalizedPayloadSource(actionSchema, payloadSource, isArrayData, isFetching, hasError) {
  if (payloadSource && !hasError && !isFetching) {
    var normalizeData = isArrayData ? payloadSource : [payloadSource];
    return (0, _normalizr.normalize)(normalizeData, [actionSchema]);
  }
  return undefined;
};
...
var makeEntitiesReducer = function makeEntitiesReducer(defaultEntitiesState) {
  return function () {
    ...
    var normalizedPayloadSource = makeNormalizedPayloadSource(actionSchema, payloadSource, isArrayData, isFetching, hasError);
